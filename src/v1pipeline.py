...
async def _perform_uploads(
    ...
    for index, (upload_path, cleanup) in enumerate(uploads):
        ...
        file_id = _extract_file_id(upload_info)
        if file_id and "path" not in upload_info:
            upload_info["path"] = file_id
...
async def _call_generator_with_existing_uploads(
    ...
    file_ids = []
    for info, _, _ in upload_infos:
        fid = _extract_file_id(info)
        if fid:
            file_ids.append(fid)
