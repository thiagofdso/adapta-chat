...
def main():
    ...
    if not current_token:
        print("Não foi possível obter o token inicial.")
        return
    ...
    for index, session_info in enumerate(sessions, start=1):
        ...
        if index > 1 and index % 10 == 0:
            print(f"\n--- Renovando token (Contagem: {index}) ---")
            new_token = get_fresh_token()
            if new_token:
                current_token = new_token
            else:
                print("Aviso: Falha ao renovar token. Tentando continuar com o antigo...")

        if sid:
            status = fix_session(sid, current_token)
            print(f"[{index}/{total}] Sessão: {sid} | Status: {status}")
