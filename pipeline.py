        ...
        for job in pending_jobs:
            ...
            try:
                ...
                if candidate is None:
                    logger.warning(
                        "Resposta sem JSON valido recebida para o job %s. Solicitando nova geracao focada apenas no JSON.",
                        job_id,
                    )
                    conversation.append({
                        'role': 'user',
                        'content': (
                            "Nao foi possivel localizar um bloco JSON valido na resposta anterior. "
                            "Responda novamente seguindo o formato solicitado e retornando apenas o JSON."
                        ),
                    })
                    _write_conversation_log(conversation)
                    continue
                ...
                if has_existing_index:
                    ...
                    if not candidate_text.strip():
                        logger.error(
                            "Resposta vazia recebida para o indice inicial do job %s. Conteudo acumulado salvo em %s",
                            job_id,
                            temp_raw_path,
                        )
                        raise ValueError("Resposta vazia ao interpretar o indice inicial.")
                    try:
                        data = json.loads(candidate_text)
                    except json.JSONDecodeError as exc:
                        debug_path = Path(INDEXES_PATH) / f'raw_job_{job_id}_initial_{int(time.time())}.json'
                        try:
                            debug_path.write_text(candidate_text, encoding='utf-8')
                            logger.error(
                                "Falha ao interpretar JSON inicial do indice: %s. Conteudo bruto salvo em %s",
                                exc,
                                debug_path,
                            )
                        except Exception as dump_exc:
                            logger.warning(
                                "Falha ao salvar conteudo bruto do job %s para depuracao: %s",
                                job_id,
                                dump_exc,
                            )
                            logger.error("Falha ao interpretar JSON inicial do indice: %s", exc)
                        logger.error("Trecho da resposta: %r", candidate_text[:200])
                        conversation.append({
                            'role': 'user',
                            'content': (
                                "O JSON retornado esta invalido. Gere novamente o conteudo seguindo exatamente o formato "
                                "solicitado e retorne apenas o objeto JSON."
                            ),
                        })
                        _write_conversation_log(conversation)
                        continue
